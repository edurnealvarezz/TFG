...
# --- Partició train/test compartida (80/20) ---
idx_train_a <- crear_o_carregar_particio(dades_rfa$Y)
train_a <- dades_rfa[idx_train_a, ]
test_a  <- dades_rfa[-idx_train_a, ]

idx_train_b <- crear_o_carregar_particio(dades_rfb$Y)
...
for (i in seq_len(nrow(grid_res))) {
  ...
  grid_res$AUC_oob[i] <- round(as.numeric(pROC::auc(pROC::roc(train_a$Y, prob_oob, quiet = TRUE))), 4)
}
...
# llindar seleccionat sobre probabilitats OOB (no contamina el test)
pr_rfa <- seleccionar_llindar_pr(prob_oob_a, train_a$Y, MIN_RECALL)
thresh_pr_a <- pr_rfa$threshold
cat(sprintf("RF-A — AUPRC (OOB): %.4f | Llindar PR: %.4f | recall_ok: %s\n\n",
...
for (i in seq_len(nrow(grid_b))) {
  ...
  grid_b$AUC_oob[i] <- round(as.numeric(pROC::auc(pROC::roc(train_b$Y, prob_oob_tmp, quiet = TRUE))), 4)
}
...
# llindar seleccionat sobre probabilitats OOB (no contamina el test)
pr_rfb <- seleccionar_llindar_pr(prob_oob_b, train_b$Y, MIN_RECALL)
thresh_pr_b <- pr_rfb$threshold
cat(sprintf("RF-B — AUPRC (OOB): %.4f | Llindar PR: %.4f | recall_ok: %s\n\n",
