# ================================================================
# 10. SuperLearner — Predicció de GRUP_ASSIST
# ================================================================
# Level 0 (7 learners): Logit, LogitMillorat, RF, XGBoost, SVM-RBF,
#                        ElasticNet, KNN
# Level 1 (meta-learner): Ridge logit (alpha=0, lower.limits=0)
#                         entrenat sobre OOF predictions del Level 0
# OOF: 10-fold CV sobre train set
# Avaluació: test set (80/20, set.seed(1234))
# MIN_RECALL = 0.60 (consistent amb script 9)
# ================================================================
...
model_logit_base <- readRDS("2. Dades/model_logit.rds")
model_logit_mil  <- readRDS("2. Dades/model_logit_millorat.rds")
formula_base     <- formula(model_logit_base)
formula_mil      <- formula(model_logit_mil)
...
cat("Formula Logit base:\n");    print(formula_base)
cat("\nFormula Logit Millorat:\n"); print(formula_mil); cat("\n")

#### ============================================================ ####
####            1. FUNCIONS DELS LEARNERS LEVEL-0                 ####
#### ============================================================ ####

# L1: Logit base (usa formula_base, necesita data.frame amb tots els predictors)
pred_logit_base_fn <- function(tr_df, te_df) {
  m <- tryCatch(glm(formula_base, data = tr_df, family = binomial),
...
for (fi in seq_along(folds)) {
  ...
  mat_oof[val_idx, "Logit"]      <- pred_logit_base_fn(df_tr, df_va)
...
oof_auc_tbl <- do.call(rbind, lapply(learner_noms, function(l) {
  ...
  auc_v <- if (sum(valid) >= 10)
    as.numeric(auc(roc(df_oof$Y[valid], vals[valid], quiet = TRUE)))
  else NA_real_
...
mat_test_l0[, "Logit"]      <- pred_logit_base_fn(train_sl, test_sl)
...
test_auc_tbl <- do.call(rbind, lapply(learner_noms, function(l) {
  ...
  auc_v <- if (sum(valid) >= 5)
    as.numeric(auc(roc(Y_test[valid], vals[valid], quiet = TRUE)))
  else NA_real_
...
# Llindar PR (MIN_RECALL = 0.60)
pr_meta    <- seleccionar_llindar_pr(prob_meta_test, Y_test_meta, MIN_RECALL)
thresh_meta <- pr_meta$threshold

cat(sprintf("AUPRC SuperLearner: %.4f\n", pr_meta$auprc))
cat(sprintf("Llindar (recall>=%.2f): %.4f | recall_ok: %s\n\n",
            MIN_RECALL, thresh_meta, ifelse(pr_meta$recall_ok, "SI", "NO (fallback)")))
...
ggplot(pr_meta$pr_curve, aes(x = recall, y = precision)) +
  ...
  geom_point(data = data.frame(
               recall    = pr_meta$recall,
               precision = ifelse(is.na(pr_meta$precision), 0, pr_meta$precision)),
...
auc_sl    <- as.numeric(auc(roc(Y_test_meta, prob_meta_test, quiet = TRUE)))
...
roc_list <- lapply(learner_noms, function(l) {
  ...
  roc_obj <- roc(Y_test_meta[valid], vals[valid], quiet = TRUE)
  data.frame(spec_inv = 1 - roc_obj$specificities,
             sens = roc_obj$sensitivities, Model = l)
})
roc_sl_df <- local({
  r <- roc(Y_test_meta, prob_meta_test, quiet = TRUE)
