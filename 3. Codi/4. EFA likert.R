...
load("2. Dades/3. Dades amb noves variables.RData")
...
# és a dir, les variables estan correlacionades i és adequat fer EFA
  ...
  p_scree <- ggplot(df_scree, aes(x = factor)) +
    ...
    annotate("text", x = n_factors + 0.6, y = max(df_scree$real) * 0.9,
             label = paste0("k = ", n_factors), hjust = 0, size = 5.5,
             color = "gray30") +
  ...
  p_comm <- ggplot(df_comm, aes(x = reorder(variable, h2), y = h2)) +
    ...
    annotate("text", x = 1.2, y = 0.31, label = "0.30", size = 5.5, color = "gray40") +
    annotate("text", x = 1.2, y = 0.51, label = "0.50", size = 5.5, color = "gray60") +
  ...
  print(
    ggplot(df_corr,
      ...
      geom_text(aes(label = sprintf("rho=%.2f %s", rho_spearman, sig)),
                hjust = ifelse(df_corr$rho_spearman >= 0, -0.1, 1.1), size = 5.5) +
...
sink("4. Outputs/4. EFA/4.1 Output_text_EFA.txt")
png("4. Outputs/4. EFA/grafic_%02d.png", width = 8, height = 6, units = "in", res = 300)
...
cat("Variables eliminades de l'EFA per comunalitats baixes: 
      M_ACAD, E_PES_AS, IA_PREOC, M_DIST, E_HORA \n")
...
dades <- afegir_scores(dades, res_motius, "FA_MOT")
dades <- afegir_scores(dades, res_estrat, "FA_EST")
dades <- afegir_scores(dades, res_ia, "FA_IA")

cat("Columnes de puntuacions afegides:\n")
cat(grep("^FA_", names(dades), value = TRUE), sep = ", ")
cat("\n\nFiles amb puntuació (sense NA):\n")
cat("  Motius:      ", sum(!is.na(dades$FA_MOT_F1)), "\n")
cat("  Estratègies: ", sum(!is.na(dades$FA_EST_F1)), "\n")
cat("  IA:          ", sum(!is.na(dades$FA_IA_F1)), "\n\n")

fer_grafic_scores(res_motius, "FA_MOT", df = dades)
fer_grafic_scores(res_estrat, "FA_EST", df = dades)
fer_grafic_scores(res_ia, "FA_IA", df = dades)

##### ------------- 2.3. CORRELACIÓ SPEARMAN AMB GRUP_ASSIST ------ #####
grafic_spearman_resum("FA_MOT", "Spearman rho factors Motius vs GRUP_ASSIST", df = dades)
grafic_spearman_resum("FA_EST", "Spearman rho factors Estratègies vs GRUP_ASSIST", df = dades)
grafic_spearman_resum("FA_IA", "Spearman rho factors IA vs GRUP_ASSIST", df = dades)

# taula resum
all_fa_cols <- grep("^FA_", names(dades), value = TRUE)
...
dades <- dades %>% select(-starts_with("FA_"))
dades <- afegir_scores(dades, res_motius_def, "FA_MOT")
dades <- afegir_scores(dades, res_estrat_def, "FA_EST")
dades <- afegir_scores(dades, res_ia_def, "FA_IA")

cat("\n\n")

fer_grafic_scores(res_motius_def, "FA_MOT", df = dades)
fer_grafic_scores(res_estrat_def, "FA_EST", df = dades)
fer_grafic_scores(res_ia_def, "FA_IA", df = dades)

##### --------- 3.2. CORRELACIÓ SPEARMAN AMB GRUP_ASSIST ------ #####

all_fa_def <- grep("^FA_", names(dades), value = TRUE)
grup_num_def <- as.integer(dades$GRUP_ASSIST == "Regular (≥80%)")

df_fa_def_corr <- lapply(all_fa_def, function(col) {
  x <- dades[[col]]
  mw <- wilcox.test(x ~ dades$GRUP_ASSIST, exact = FALSE, na.action = na.omit)
